...
function SlippyRouteMap({ flight, navFixes, airports, compact, showLabels, onToggleLabels }) {
  ...
  return (
    <div ref={containerRef} className={`relative h-full w-full overflow-hidden bg-[#E8E8E3] ${compact ? "rounded-md" : ""}`}>
      {tiles.map((tile) => (
        <img key={`${tile.x}-${tile.y}-${tile.z}`} src={tile.url} alt="" className="pointer-events-none absolute z-0 select-none" style={{ left: tile.left, top: tile.top, width: 256, height: 256, opacity: 0.86, filter: "saturate(86%) contrast(96%) brightness(102%)" }} draggable={false} loading="eager" />
      ))}
      ...
      <div className="absolute bottom-1 right-1 rounded bg-white/82 px-1.5 py-0.5 text-[8px] text-gray-500">© CARTO · © OpenStreetMap contributors</div>
    </div>
  );
}
...
function makeTiles(zoom, bounds, width, height) {
  ...
  for (let y = Math.max(0, minY - 2); y <= Math.min(tileCount - 1, maxY + 2); y += 1) {
    for (let x = minX - 2; x <= maxX + 2; x += 1) {
      ...
      tiles.push({ z: zoom, x: wrappedX, y, left: x * 256 - topLeft.x, top: y * 256 - topLeft.y, url: `https://basemaps.cartocdn.com/rastertiles/voyager/${zoom}/${wrappedX}/${y}.png` });
    }
  }
  return tiles;
}
